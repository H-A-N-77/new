...
class ReplayBuffer(object):
    ...
    def save_buffer_to_file(self, filename, directory, buffer_name='general'):
        store_item_list = []
        print("vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv")
        print("buffer_name:{}self.storage:{}".format(buffer_name, len(self.storage)))
        storage = self.storage
        eposide_rewad = self.eposide_rewad
        storage_array = np.array(storage)
        ptr = self.ptr
        eposide_rewad_array = np.array(eposide_rewad)
        npy_ptr = os.path.join(directory, filename + '_' + buffer_name + 'ptr.npy')
        npy_file_name = os.path.join(directory, filename + '_' + buffer_name + 'buffer.npy')
        npy_eposide_rewad_file_name = os.path.join(directory, filename + '_' + buffer_name + 'eposide_rewad.npy')
        np.save(npy_ptr, ptr)
        np.save(npy_file_name, storage_array)
        np.save(npy_eposide_rewad_file_name, eposide_rewad_array)

    def load_buffer_from_file(self, filename, directory, buffer_name='general'):
        npy_ptr = os.path.join(directory, filename + '_' + buffer_name + 'ptr.npy')
        npy_file_name = os.path.join(directory, filename + '_' + buffer_name + 'buffer.npy')
        npy_eposide_rewad_file_name = os.path.join(directory, filename + '_' + buffer_name + 'eposide_rewad.npy')
        ptr = np.load(npy_ptr, allow_pickle=True)
        storage_array = np.load(npy_file_name, allow_pickle=True)
        eposide_rewad_array = np.load(npy_eposide_rewad_file_name, allow_pickle=True)
        self.ptr = ptr
        self.storage = list(storage_array)
        self.eposide_rewad = list(eposide_rewad_array)
        # print("storage_array:", self.storage[0])

        print("Load Replay_Buffer Success!!!")
    ...
    def sample(self, batch_size):
        ...
        for i in ind:
            O, X, Y, O_, U, R, D = self.storage[i]
            # print("type:", type(O[0]))
            # print("is?:", isinstance(O[0], np.ndarray))

            # if isinstance(O[0], np.ndarray):
            #     for index in range(len(O)):
            #         O[index] = O[index].tolist()
...
class embedding_Buffer(object):
    ...
    def save_buffer_to_file(self, filename, directory, buffer_name='good'):
        store_item_list = []
        print("vvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvvv")
        print("buffer_name:{}self.storage:{}".format(buffer_name, len(self.pos_storage)))
        ...
        json_file_name = os.path.join(directory, filename+'_'+buffer_name + 'buffer.json')
        npy_ptr = os.path.join(directory, filename + '_' + buffer_name + 'ptr.npy')
        np.save(npy_ptr, self.ptr)
        npy_pos_ptr = os.path.join(directory, filename + '_' + buffer_name + 'pos_ptr.npy')
        np.save(npy_pos_ptr, self.pos_ptr)
        npy_pos_storage_reward = os.path.join(directory, filename + '_' + buffer_name + 'pos_storage_reward.npy')
    ...
    def load_buffer_from_file(self, filename, directory, buffer_name='good'):
        ...
        ptr = np.load(npy_ptr, allow_pickle=True)
        self.ptr = ptr
        npy_ptr = os.path.join(directory, filename + '_' + buffer_name + 'pos_ptr.npy')
        pos_ptr = np.load(npy_ptr, allow_pickle=True)
        self.pos_ptr = pos_ptr
        npy_pos_storage_reward = os.path.join(directory, filename + '_' + buffer_name + 'pos_storage_reward.npy')
        pos_storage_reward = np.load(npy_pos_storage_reward, allow_pickle=True)
        self.pos_storage_reward = list(pos_storage_reward)
        json_file_name = os.path.join(directory, filename+'_'+buffer_name + 'buffer.json')
        with open(json_file_name, 'r') as f:
            store_item_list = json.load(f)
        print("storage size:", len(store_item_list))
        storage = []
        for store_item in store_item_list:
            store_item = tuple(np.array(arr) for arr in store_item)
            storage.append(store_item)
        self.pos_storage = storage
